...
SELECT category, MIN(price) AS min_price FROM products GROUP BY category HAVING MIN(price) > 5000;

SELECT city, COUNT(*) AS total_orders FROM orders GROUP BY city HAVING COUNT(*) >= 2 ORDER BY total_orders DESC;
